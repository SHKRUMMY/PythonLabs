...
class CircularLinkedList:
        ...
        if self.head is None:
            ...
            while True:
                if curr.measurement_limit == specific_measurement_limit:
                    flag_val = True
                    print(f"The element with specific measurement limit {specific_measurement_limit} is present in list at position : {str(i)}, "
    ...
    def print_ll(self):
        ...
        if self.head is not None:
            while True:
                print(f"Device type:{temp.device_type}, device relase year: {temp.release_year}, device mark: {temp.mark}, device measurement_limit {temp.measurement_limit}")
                temp = temp.next
                if temp == self.head:
                    break
...
my_ll.push_back("p-n-p", "B-16", 1, 2020)
my_ll.push_back("p-n-p", "F-2", 0.5, 2022)
my_ll.push_back("n-p-n", "C-4", 1.3, 2017)
my_ll.push_back("dual", "DL-1", 0.5, 2021)
